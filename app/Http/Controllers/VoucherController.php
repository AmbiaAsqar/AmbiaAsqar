...
class VoucherController extends Controller
{
    ...
    public function confirm(Request $request)
    {
        ...
        if(!$voucher) return response()->json(['status' => false, 'message' => 'Kode voucher nya tidak ditemukan'], 404);
        if($voucher->stock == 0) return response()->json(['status' => false, 'message' => 'Voucher nya sudah tidak valid'], 404);
        ...
        if(isset($request->service)){
           ...
           $service->harga = $service->harga - $potongan;
            ...
            return response()->json([
                'status' => true,
                'message' => 'Anda Mendapatkan Potongan Rp '. number_format($potongan, 0, '.', ','),
