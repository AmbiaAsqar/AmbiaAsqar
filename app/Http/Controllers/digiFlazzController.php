...
class digiFlazzController extends Controller
{
    public function order($uid = null, $zone = null, $service = null, $order_id = null)
    {
        ...
        //get format pemisah digiflazz
        //$getFormat = \DB::select('kategoris.format_pemisah')->table('layanans')->join('kategoris', function ($join) {$join->on('kategoris.id', '=', 'layanans.ketegori_id')->where('layanans.provider_id', '=', $service);})->first();

        $getFormat = \DB::table('layanans')
            ->join('kategoris', 'kategoris.id', '=', 'layanans.kategori_id')
            ->select('kategoris.format_pemisah')
            ->where('layanans.provider_id', $service)
            ->first();

        $target = ($getFormat->format_pemisah == null) ? $uid . $zone : $uid . $getFormat->format_pemisah . $zone;
    ...
    public function cekSaldo()
    {
        ...
        return $this->connect('/v1/cek-saldo', $data, $header)['data']['deposit'];
    }
