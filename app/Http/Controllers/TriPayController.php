...
{
    public function request($idOrder, $jumlah, $method, $dataUser, $nohp, $qty)
    {
        ...
        $open = fopen('tripay_response'.date('YmdH').'.txt', 'a');
        fwrite($open, json_encode($response));
        fclose($open);
        // dd($response);
        ...
        if($response->success == true){
            ...
            if(str_contains($err, 'minimum')) {
                $pch = explode('rp',$err);
                $msg = 'Minimum jumlah pembayaran ini Rp '.$pch[1].' ';
            }elseif(str_contains($err, 'maximum')){
                $pch = explode('rp',$err);
                $msg = 'Maksimal jumlah pembayaran ini Rp '.$pch[1].' ';
            }else{
