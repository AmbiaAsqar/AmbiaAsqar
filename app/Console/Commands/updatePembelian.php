            ...
            foreach ($datas as $data) {
                ...
                try{
                    ...
                    if($pembelian->tipe_transaksi == "game"){
                        $layanan = Layanan::where('layanan', $pembelian->layanan)->select('provider_id', 'kategori_id', 'provider', 'sisa', 'event')->first();
                    ...
                    foreach ($list_transaksi as $transfer) {
                        if ($transfer['type'] == "credit" && $transfer['amount']['value'] == $pembayaran->harga) {
                            ...
                            try {
                                ...
                                Pembayaran::where('order_id', $data->order_id)->update(['status' => 'Paid']);
                                
                                 if($layanan->event == 1){
                                    $sisa = $layanan->sisa - 1;
                                    $updatesisa = Layanan::where('layanan', $pembelian->layanan)->update(['sisa' => $sisa]);
                                }
                                ...
                                if($layanan->provider == "digiflazz"){
                                    ...
                                    if ($order['data']['status'] == "Pending" || $order['data']['status'] == "Proses") {
                                        $order['status'] = true;
                                    } else {
                                        $order['status'] = false;
                                    }   
                                ...
                                if($pembelian->tipe_transaksi !== 'joki'){
                                    $updatePembelian = Pembelian::where('order_id', $data->order_id)->update(['status' => 'Proses']);
                                }
                            } catch (\Exception $e) {
                                continue;
                            }
